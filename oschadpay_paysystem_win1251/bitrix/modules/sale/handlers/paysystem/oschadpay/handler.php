...
/**
 * Class OschadpayHandler
 * @package Sale\Handlers\PaySystem
 */
class OschadpayHandler extends PaySystem\BaseServiceHandler implements PaySystem\IRefundExtended
{
    /**
    ...
    {
        return array('BX_HANDLER' => 'OSCHADPAY');
    }
    /**
     * @param Payment $payment
    ...
    {
        $busValues = $this->getParamsBusValue($payment);
        $url = 'https://api.oschadpay.com.ua/api/reverse/order_id';
        $error = '';
        ...
        if ($busValues["PREAUTH"] != 'Y'){
            $error = Loc::getMessage('OSCHADPAY_REFUND_ERROR_HOLD_DSBLD');
		}
        ...
        if ($content !== false ) {
            ...
            elseif($error == '')
                $error .= Loc::getMessage('OSCHADPAY_REFUND_ERROR') . ' ' . Loc::getMessage('OSCHADPAY_REFUND_ERROR_INFO', array('#STATUS#' => $content['response']['response_status'],'#REQ_ID#' => $content['response']['request_id'], '#ERROR#' => $content['response']['error_message']));
        } else {
            $error .= Loc::getMessage('OSCHADPAY_REFUND_CONNECTION_ERROR', array('#URL#' => $url, '#ERROR#' => $curlError ? $curlError : $content['response']['error_message'], '#CODE#' => $httpCode));
        }
    ...
    public static function findMyDataRefundablePage(array $paySystemList)
    {
        ...
        foreach ($paySystemList as $data) {
            foreach ($personTypeList as $personType) {
                $shopId = BusinessValue::get('OSCHADPAY_SHOP_ID', PaySystem\Service::PAY_SYSTEM_PREFIX . $data['ID'], $personType['ID']);
