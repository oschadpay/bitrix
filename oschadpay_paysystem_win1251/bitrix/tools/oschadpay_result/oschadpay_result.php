...
if (CModule::IncludeModule('sale')) {
	...
	require $_SERVER['DOCUMENT_ROOT'] .  "/bitrix/modules/sale/payment/oschadpay/oschadpay.cls.php";

	$oschadpayOpt = array();
	$b = unserialize($payData['PARAMS']);
	foreach ($b as $k => $v) $oschadpayOpt[$k] = $v['VALUE'];

	$oschadpay = new Oschadpay();
	$oschadpayResult = $oschadpay->isPaymentValid($oschadpayOpt, $_POST);

	if ($_REQUEST['order_status'] != Oschadpay::ORDER_APPROVED) {
		$answer = 'declined';
	} elseif ($oschadpayResult == true) {
		$answer = 'OK';
		CSaleOrder::PayOrder($ORDER_ID, 'Y');
	} else {
		$answer = $oschadpayResult;
	}
	$arOrder = CSaleOrder::GetByID($ORDER_ID);
	if ($arOrder) {
		$arFields = array(
			...
			"PS_INVOICE_ID" => $_POST['order_id'],
			"PS_STATUS_CODE" => $_POST['order_id'],
			"PS_STATUS_DESCRIPTION" => $_POST['order_status'] . " " . $payID . " " .
				($answer != 'OK' ? $_POST['response_description'] : ''),
			"PS_STATUS_MESSAGE" => $_POST['order_status'],
			"PS_SUM" => $_POST['amount'],
